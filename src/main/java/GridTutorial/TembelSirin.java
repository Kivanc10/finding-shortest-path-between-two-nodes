...
public class TembelSirin extends Oyuncu {
    ...
    public TembelSirin(int oyuncuId, String oyuncuAdi, String oyuncuTur, int skor, int x, int y) {
        super(oyuncuId, oyuncuAdi, oyuncuTur, skor, x, y);
        System.out.println("tembel bounds : " + this.charJLabel.getBounds());
    }

    public TembelSirin(int oyuncuId, String oyuncuAdi, String oyuncuTur, int skor) {
    ...
    public void typeOfAdvanceOnMaze(Oyuncu player, JFrame f, JLabel jlabel2, ArrayList<Rectangle> available_coords, ArrayList<Dusman> enemies) {
        ...
        player.setImageIcon("D:\\PROJECTS\\NetBeansProjects\\GridTutorial\\tembel.jpeg");
        ...
        f.addKeyListener(new KeyListener() {
            ...
            public void keyPressed(KeyEvent e) {
                ...
                switch (keyCode) {
                    case KeyEvent.VK_UP:
                        step_temp = new Rectangle(player.charJLabel.getBounds().x, player.charJLabel.getBounds().y - 80, 80, 80);
                        if (available_coords.contains(step_temp)) {
                            player.charJLabel.setBounds(player.charJLabel.getBounds().x, player.charJLabel.getBounds().y - 80, 80, 80);
                            player.setXLoc(player.charJLabel.getBounds().y / 80);
                            //System.out.println("palyer charjlabel : " + player.charJLabel.getBounds());
...
//                            System.out.println("tembel s. koord : " + player.getXLoc() + "," + player.getYLoc());
                            ...
                            if (enemies.size() == 1) {
                                ...
                                d1.enKisaYol(player);
                                if (d.getXLoc() == player.getXLoc() && d.getYLoc() == player.getYLoc()) {
                                    player.skorLabel.setText("yakalandï¿½n");
                                    if (d.name().startsWith("G")) {
                                        if (player.getSkor() - 15 > 0) {
                                            player.setSkor(player.getSkor() - 15);
                                            d.charJLabel.setBounds(d.getFinalY() * 80, d.getFinalX() * 80, 80, 80);
                    ...
                    case KeyEvent.VK_RIGHT:
                        step_temp = new Rectangle(player.charJLabel.getBounds().x + 80, player.charJLabel.getBounds().y, 80, 80);
                        if (available_coords.contains(step_temp)) {
                            player.charJLabel.setBounds(player.charJLabel.getBounds().x + 80, player.charJLabel.getBounds().y, 80, 80);
                            player.setXLoc(player.charJLabel.getBounds().y / 80);
                            player.setYLoc(player.charJLabel.getBounds().x / 80);
                            System.out.println("tembel s. koord : " + player.getXLoc() + "," + player.getYLoc());
                            player.goldQuery();
    ...
    public void PuaniGoster(Oyuncu player) {
        ...
        player.skorLabel.setText("Tembel sirin puani " + this.getSkor());
